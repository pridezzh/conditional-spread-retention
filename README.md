...
│
├── paper/
│   ├── main.tex             manuscript source (ICLR 2027 style, <=9 pp main text)
│   ├── numbers_theory.tex   GENERATED: every paper number is a macro here
│   ├── references.bib       bibliography
│   ├── iclr2027/            official style files (used by the build pipeline)
│   └── fetch_refs.py        regenerates bibliography from arXiv metadata
│       make_bib.py
│
├── code/
...
│
├── results/                 experiment output (6 JSON files; what the paper cites)
├── logs/                    verification results (7 JSON files; feed the macros)
└── figures/                 fig_impossibility.pdf, fig_loss_ladder.pdf
```

Compiled `paper/main.pdf`, text process logs (`logs/*.log`), figure PNGs, and
superseded scripts are **not shipped** — they are regenerated locally.

---

## How numbers are traced

No statistic in the paper is hand-copied. Every value is a LaTeX macro in
`paper/numbers_theory.tex`, generated by `code/analysis/make_theory_macros.py` from
`results/*.json` and `logs/*.json`. Each macro line ends with a comment naming its
