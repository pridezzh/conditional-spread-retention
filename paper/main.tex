% =====================================================================
%  When Does One-Step Endpoint Regression Preserve Conditional Spread?
%  -- ICLR 2027 submission draft (anonymous)
%  编译: pdflatex -> bibtex -> pdflatex -> pdflatex（推荐用 code/analysis/build_paper.py）
%  数值宏全部来自 numbers_theory.tex，由 code/analysis/make_theory_macros.py
%  从 logs/*.json 与 results/*.json 自动生成，勿手改。
%  （旧版的 numbers.tex 与其流水线已归档到 _archive_20260916/。）
% =====================================================================
\documentclass{article}
\usepackage{iclr2027_conference,times}
% hypertexnames=false prevents duplicate PDF anchors when floats are deferred across
% the main-text/appendix boundary by the conference style.
\usepackage[hidelinks,hypertexnames=false]{hyperref}
\usepackage{url}
\usepackage{amsmath,amssymb,booktabs,multirow}
\usepackage{amsthm}
\usepackage{graphicx}
\usepackage{xcolor}
\usepackage{adjustbox}
% 表格统一缩小到栏宽。第三轮重写后论文只有 2 张手写的小表，
% 不再经过 tables/*_body.tex（那批旧表在 _archive_20260916/ 里）。
\newcommand{\tblstyle}[1]{%
  \begin{adjustbox}{max width=\columnwidth}%
  \footnotesize\setlength{\tabcolsep}{4pt}%
  #1%
  \end{adjustbox}}
\usepackage{algorithm}
\usepackage{algorithmic}

% The main text must fit the ICLR 2027 nine-page limit, so float separation and
% caption skips are tightened from the style defaults (20/12/12pt and 10pt).
\setlength{\textfloatsep}{5pt plus 2pt minus 2pt}
\setlength{\floatsep}{5pt plus 2pt minus 2pt}
\setlength{\intextsep}{5pt plus 2pt minus 2pt}
\setlength{\abovecaptionskip}{3pt}
\setlength{\belowcaptionskip}{0pt}

% Display-math skips are tightened from the 10pt class defaults for the same reason.
\setlength{\abovedisplayskip}{4pt plus 2pt minus 2pt}
\setlength{\belowdisplayskip}{4pt plus 2pt minus 2pt}
\setlength{\abovedisplayshortskip}{2pt plus 1pt}
\setlength{\belowdisplayshortskip}{2pt plus 1pt}

% 正文所有实验数字都来自这里；由 code/analysis/make_theory_macros.py 生成。
% （旧版的 numbers.tex 已不被引用，见 _archive_20260916/。）
\input{numbers_theory.tex}

\theoremstyle{plain}
\newtheorem{theorem}{Theorem}
\newtheorem{prop}{Proposition}
\newtheorem{cor}{Corollary}

\newcommand{\todo}[1]{\textbf{\textcolor{red}{[#1]}}}
\newcommand{\Real}{\mathbb{R}}
\newcommand{\cD}{\hat D}
\newcommand{\cL}{\hat\Lambda}
\newcommand{\tr}{\operatorname{tr}}
\newcommand{\Var}{\operatorname{Var}}
\newcommand{\Cov}{\operatorname{Cov}}
\newcommand{\Eop}{\mathbb{E}}
\newcommand{\Ncal}{\mathcal{N}}
\newcommand{\supp}{\operatorname{supp}}
\newcommand{\indep}{\perp\!\!\!\perp}

\title{When Does One-Step Endpoint Regression Preserve Conditional Spread?\\
Coupling, Mean Dependence, and Data-Side Non-Identifiability}

\author{Anonymous Author(s)}

%\iclrfinalcopy
\begin{document}
\maketitle

%======================================================================
\begin{abstract}
Few-step generative policies can either match iterative samplers or collapse. We isolate
one narrower question: how much conditional total variance is retained by an
$L^2$-optimal endpoint regressor? Its ratio $\rho^\ast$ is the squared multivariate
correlation ratio between source and target, conditional on context---a classical
explained-variance quantity, not a new dependence measure. It gives an exact criterion:
collapse occurs under \emph{conditional mean independence}; full statistical independence
is sufficient but not necessary. We construct a one-parameter family of couplings with the
same observed $(c,x_1)$ marginal for every $\alpha$, while $\rho^\ast(\alpha)=\alpha^2$
sweeps $[0,1]$. On it, any estimator restricted to target-side data has minimax
absolute-error risk $1/2$, whereas the paired $(x_0,x_1,c)$ samples used by training
identify $\rho^\ast$. We also compare pointwise, pooled Chamfer, within-condition Chamfer,
and balanced assignment objectives in a controlled ring-mixture task: matching scope
strongly changes conditional spread here, but establishes no universal ranking of losses
or general sample quality. All experiments are CPU-only---synthetic families plus a
real-image MNIST endpoint study---with no claim about downstream return or robot
performance.
\end{abstract}

%======================================================================
\section{Introduction}
\label{sec:intro}

Generative policies---diffusion and flow-matching action heads---are a default way to
represent multimodal behaviour in continuous control and vision-language-action models
\citep{chi2023diffusionpolicy,wang2023diffusionql}. Their cost is iterative sampling,
so a large literature compresses sampling to one or a few network evaluations:
consistency models \citep{song2023consistency}, shortcut models
\citep{frans2024shortcut}, mean flows \citep{geng2025meanflows}, rectified-flow reflow
\citep{liu2023rectifiedflow}, distillation \citep{salimans2022progressivedistill,
yin2024dmd}, and in RL one-step flow policies \citep{nguyen2026bfq,li2026ofp,mvp2026}.

The reported picture is inconsistent in an instructive way: one-step policies are said
to match $100$-step diffusion on $56$ manipulation tasks \citep{li2026ofp} and to
reach $92.8$ on D4RL MuJoCo \citep{nguyen2026bfq}, yet the same families degrade when
the conditioning is weakened or the action horizon lengthened
\citep{li2026ofp,letitbesimple2026}. The most explicit diagnosis frames one-step
success as a property of the condition--target \emph{structure}
\citep{letitbesimple2026}, and states as an open problem the task of \emph{quantifying}
when it holds.

This paper narrows that open problem. Prior work proves collapse for an independent
pairing; we show the question is not decidable from the target marginal at all. A single
variance ratio cannot decide overall sample quality, but it can characterize mean collapse
for endpoint regression, and that quantity is not identified by $p(c,x_1)$ without the
source--target pairing used in training.

\paragraph{What we mean by conditional-spread retention.}
For a one-step map $f$ we define the realised conditional-spread ratio
\begin{equation}
  \rho(f) \;=\;
  \frac{\Eop_c\big[\tr\Var_{x_0}\big(f(x_0,c)\mid c\big)\big]}
       {\Eop_c\big[\tr\Var(x_1\mid c)\big]} \;\in\;[0,\infty),
  \label{eq:rho}
\end{equation}
the ratio of generated to target conditional total variance, assuming the denominator is
positive and finite. $\rho\approx0$ signals collapse onto a per-condition point mass;
$\rho\approx1$ only matches aggregate spread and does \emph{not} imply the correct
conditional law, mode weights, or sample quality. For the $L^2$ population optimum,
Thm.~\ref{thm:optimal} shows $\rho\in[0,1]$.

\paragraph{Contributions.}
\begin{enumerate}
\item[\textbf{C1}] \textbf{An exact mean-dependence characterization.} The $L^2$-optimal
endpoint map is $f^\ast(x_0,c)=\Eop_\pi[x_1\mid x_0,c]$, and $\rho(f^\ast)$ is the
squared multivariate correlation ratio \citep{renyi1959,sampson1984}. It is zero exactly
under conditional mean independence and one under a deterministic coupling
(Thm.~\ref{thm:optimal}). This corrects the stronger but false implication that every
non-independent coupling is non-degenerate.
\item[\textbf{C2}] \textbf{An impossibility theorem with a paired-data escape.} There is a
one-parameter family of training targets with an \emph{identical} $(c,x_1)$ law for
which $\rho^\ast(\alpha)=\alpha^2$ sweeps $[0,1]$ (Thm.~\ref{thm:alpha}); hence
$\inf_{\mathcal A}\sup_\alpha\Eop|\mathcal A-\rho^\ast(\alpha)|=\MinimaxConst$ over
\emph{all} data-only statistics, attained by the constant $1/2$
(Thm.~\ref{thm:minimax})---more data does not help. The paired samples used by
training identify $\rho^\ast$ in the population sense---it is a functional of the paired
law---and under an ideal local-neighbour approximation the finite-$k$ curve is
$\alpha^2+(1-\alpha^2)/k$, which the measured curve tracks to MAD \ImpPredMad{}
(Sec.~\ref{sec:estimator}).
\item[\textbf{C3}] \textbf{A controlled loss comparison.} Population objectives explain
why pointwise $L^2$, unbalanced support matching, and balanced transport have different
sets of optima. In our ring task, pooling candidates across conditions flattens
per-condition spread, while within-condition matching covers all evaluated modes; the
balanced variant has lower conditional sliced $W_2$ in five paired seeds. This is a
task-specific result, not a universal ordering.
\item[\textbf{C4}] \textbf{A real-data confirmation with a constructed pairing.} On MNIST
$16{\times}16$ endpoint regression from Gaussian noise, a coupling that is measurably
dependent but conditionally mean-independent collapses exactly like the independent one
(realised spread \MnistBRhoTr{} versus \MnistARhoTr{}; distance correlation
\MnistBDcor{} versus \MnistADcor{}), mini-batch OT does not (\MnistCRhoTr{}), and a
sorted quantile pairing that forces mean dependence through one statistic lands in
between, as prescribed (\MnistDRhoTr{}; Sec.~\ref{sec:mnist}). Dependence per se
does not rescue one-step regression on real images; making the conditional mean
depend on $x_0$ does.
\end{enumerate}

\paragraph{Scope and honesty statement.} Every experiment is CPU-only, and every claim is
either an exact theorem about what \emph{cannot} be predicted or a controlled measurement
of what \emph{does} happen under a given training target, checked against pre-registered
criteria. Appendix~\ref{app:scope} gives the full statement, including an earlier
under-fitted \emph{conditional}-MNIST iteration on which nothing here depends.

%======================================================================
\section{Setup and the classical dependence quantity}
\label{sec:setup}

Let $c\in\mathcal C$ be a conditioning variable (observation, state, goal, embedding)
and $x_1\in\Real^d$ a target (action, action chunk, image), with joint law
$p(c,x_1)$ observed only through i.i.d.\ samples. A one-step generative policy is a
map $f:\Real^d\times\mathcal C\to\Real^d$ applied to source noise
$x_0\sim\Ncal(0,I_d)$; a flow or diffusion policy instead integrates a velocity field
learned from a \emph{coupling} $\pi$ between $x_0$ and $x_1$. Throughout, $\pi$ is the
joint law of $(x_0,x_1)$ given $c$, with both marginals fixed: $x_0\sim\Ncal(0,I_d)$
and $x_1\sim p(\cdot\mid c)$. The two couplings that matter in practice are the
\emph{independent} coupling $x_0\indep x_1\mid c$ used by vanilla conditional flow
matching \citep{lipman2023flowmatching,tong2024cfm}, and a \emph{deterministic}
coupling $x_1=T(x_0,c)$, which is what reflow \citep{liu2023rectifiedflow} and
consistency distillation produce.

\begin{theorem}[Optimal endpoint map and mean-dependence criterion]
\label{thm:optimal}
Assume $\Eop\|x_1\|^2<\infty$ and $0<\Eop_c\tr\Var(x_1\mid c)<\infty$, and fix a coupling
$\pi$. The minimiser of the pointwise $L^2$ risk $\Eop_\pi\|f(x_0,c)-x_1\|^2$ over all $f$
with $\Eop\|f(x_0,c)\|^2<\infty$ is
$f^\ast(x_0,c)=\Eop_\pi[x_1\mid x_0,c]$, and
\[
  \rho(f^\ast)=
  \frac{\Eop_c\big[\tr\Var\big(\Eop[x_1\mid x_0,c]\mid c\big)\big]}
       {\Eop_c\big[\tr\Var(x_1\mid c)\big]}\;\in\;[0,1],
\]
with the bound following from the law of total variance,
$\Var(x_1\mid c)=\Eop[\Var(x_1\mid x_0,c)\mid c]+\Var(\Eop[x_1\mid x_0,c]\mid c)$.
Moreover, $\rho(f^\ast)=0$ iff $\Eop[x_1\mid x_0,c]=\Eop[x_1\mid c]$ almost surely
(conditional mean independence), and $\rho(f^\ast)=1$ iff
$\Var(x_1\mid x_0,c)=0$ almost surely.
\end{theorem}
\begin{proof}[Proof sketch]
The conditional mean is the $L^2$ projection; the law of total variance gives the
decomposition and hence $\rho\le1$; $\rho\ge0$ is trivial. Appendix~\ref{app:proofs}.
\end{proof}

The ratio in Thm.~\ref{thm:optimal} is the squared multivariate correlation ratio
\citep{renyi1959,sampson1984}; neither the projection identity nor the measure is new.
Its use here distinguishes full independence from the weaker, exact condition that
matters for squared-loss endpoint collapse: mean independence.

\begin{cor}[Independent coupling collapses]
\label{cor:indep}
If $x_0\indep x_1\mid c$ then $\Eop[x_1\mid x_0,c]=\Eop[x_1\mid c]=m(c)$ is
constant in $x_0$, so $\rho(f^\ast)=0$: the optimal one-step map is a per-condition
point mass.
\end{cor}

The converse of Cor.~\ref{cor:indep} is false. For example, with no context, let
$x_0\sim\Ncal(0,1)$, let $S$ be an independent Rademacher sign, and set
$x_1=S|x_0|$. Then $x_1\sim\Ncal(0,1)$ and the coupling is dependent because
$|x_1|=|x_0|$, yet $\Eop[x_1\mid x_0]=0$ and $\rho(f^\ast)=0$. Thus a
non-independent coupling need not prevent mean collapse.

\begin{cor}[Deterministic coupling does not]
\label{cor:det}
If $x_1=T(x_0,c)$ with $T(\cdot,c)_\#\Ncal(0,I_d)=p(\cdot\mid c)$, then
$f^\ast=T$ and $\rho(f^\ast)=1$.
\end{cor}

\paragraph{Corollary~\ref{cor:indep} in closed form.} We verify the endpoint identity
numerically on the exact velocity field of a Gaussian-mixture conditional, where
$v^\ast(x,0,c)=\Eop[x_1\mid c]-x$ can be integrated in closed form: the unit Euler
endpoint matches $m(c)$ to $\MSEndpointErr$ and realises conditional variance
$\MSVarOne$, while the $\ImpK$-mode ring is covered $0$ times at $N=1$ and
\MSCovTwo{}/\ImpK{} from $N=2$ on. The multi-step ratio rises monotonically in the step count, from $\MSRhoOne$ at $N=1$
to $\MSRhoTwoFiftySix$ at $N=256$ (all ten values in Appendix~\ref{app:proofs}).
Under a conditional (multi-condition) version of the same
construction, changing \emph{only} the coupling moves one-step $\rho$ from
\CondRhoInd{} to \CondRhoDet{} at $k=\CondKnnK$---a factor of \CondContrast---while
the data statistic $\cD$ takes the value $\CondDIndep$ in \emph{both} arms, to a
measured difference of $\CondDDiff$. That invariance is exact rather than a near-miss
measurement: $\cD$ is a functional of the
$(c,x_1)$ pairs alone, and switching the coupling only reorders those pairs within each
condition.

\paragraph{The exact pointwise-$L^2$ criterion.} For endpoint regression,
Thm.~\ref{thm:optimal} gives
\begin{equation}
  \rho(f^\ast)=0\quad\Longleftrightarrow\quad
  \Eop[x_1\mid x_0,c]=\Eop[x_1\mid c]\quad\text{a.s.}
  \label{eq:channels}
\end{equation}
Independent pairing is one sufficient route to the right-hand side; deterministic
transport attains the other endpoint. Changing the loss can also avoid the pointwise
conditional mean, but no universal converse is claimed.

%======================================================================
\section{The data cannot decide}
\label{sec:imposs}

The two channels in \eqref{eq:channels} are properties of how you train. A natural
hope is that they are nevertheless \emph{predictable} from the data: that some
statistic of a dataset of $(c,x_1)$ pairs tells you in advance whether one step will
suffice. This section shows the hope is not merely hard but void, and quantifies
exactly how void it is.

\subsection{A family with fixed data and sweeping \texorpdfstring{$\rho$}{rho}}
\label{sec:alphafam}

Fix $p(\cdot\mid c)$, assumed atomless so that a transport map exists, and let
$T(\cdot,c)$ push $\Ncal(0,I_d)$ to $p(\cdot\mid c)$---concretely, the flow map of the
exact velocity field, i.e.\ what reflow would produce. For $\alpha\in[0,1]$ define the
\emph{$\alpha$-interpolated coupling}
\begin{equation}
  x_1 \;=\; B\cdot T(x_0,c) + (1-B)\cdot X,\qquad
  B\sim\mathrm{Bernoulli}(\alpha),\quad X\sim p(\cdot\mid c),\ X\indep x_0\mid c .
  \label{eq:alpha}
\end{equation}

\begin{theorem}[Fixed data, sweeping $\rho$]
\label{thm:alpha}
For the coupling \eqref{eq:alpha}, with $\Eop\|x_1\|^2<\infty$: \emph{(i)} both branches have marginal
$p(\cdot\mid c)$, so the law of $(c,x_1)$ does not depend on $\alpha$; \emph{(ii)}
$f^\ast_\alpha(x_0,c)=\alpha T(x_0,c)+(1-\alpha)m(c)$; and \emph{(iii)}
$\rho^\ast(\alpha)=\alpha^2$.
\end{theorem}
\begin{proof}
(i) $T(x_0,c)\sim p(\cdot\mid c)$ because $T$ is a transport map, and
$X\sim p(\cdot\mid c)$ by construction; a mixture of two identical laws is that law.
(ii) $\Eop[x_1\mid x_0,c]=\alpha\Eop[T(x_0,c)\mid x_0,c]+(1-\alpha)\Eop[X\mid c]$.
(iii) Given $c$, the conditional variance of $\alpha T(x_0,c)+(1-\alpha)m(c)$ over
$x_0$ is $\alpha^2\Var(x_1\mid c)$, since the second term is constant and
$\Var(T(x_0,c)\mid c)=\Var(x_1\mid c)$.
\end{proof}

The family is not exotic: $\alpha=0$ is the standard independent coupling of vanilla
conditional flow matching, $\alpha=1$ is the deterministic coupling that reflow
produces, and intermediate $\alpha$ is a mixture of the two. What varies is a design
choice; what does not vary is the law of a single observed pair $(c_i,x_{1,i})$.

\subsection{Minimax form: the target marginal is insufficient}
\label{sec:minimax}

\begin{theorem}[Impossibility, minimax form]
\label{thm:minimax}
Let $\mathcal A$ be any estimator of $\rho^\ast$ measurable with respect to $n$
i.i.d.\ draws from $p(c,x_1)$---randomised, and allowed to know the population law
$p(c,x_1)$ exactly. Then for the family of Theorem~\ref{thm:alpha},
\[
  \inf_{\mathcal A}\ \sup_{\alpha\in[0,1]}\ \Eop_\alpha\big|\mathcal A-\rho^\ast(\alpha)\big|
  \;=\; \tfrac12 \;=\; \inf_{a\in\Real}\ \sup_{\alpha\in[0,1]}\big|a-\alpha^2\big| ,
\]
and the infimum on the left is attained by the constant $\mathcal A\equiv1/2$.
\end{theorem}
\begin{proof}
By Theorem~\ref{thm:alpha}(i) the law of the sample is the same for every $\alpha$, so
the law of $\mathcal A$---and in particular $\bar a:=\Eop[\mathcal A]$---does not
depend on $\alpha$. By Jensen,
$\Eop_\alpha|\mathcal A-\alpha^2|\ge|\bar a-\alpha^2|$ for each $\alpha$, hence
$\sup_\alpha\Eop_\alpha|\mathcal A-\alpha^2|\ge\sup_\alpha|\bar a-\alpha^2|
\ge\max\{|\bar a|,|\bar a-1|\}\ge1/2$, the last step because $\alpha^2$ attains both
$0$ and $1$. Conversely $\mathcal A\equiv1/2$ achieves
$\sup_{\alpha}|1/2-\alpha^2|=1/2$. The second equality is the same argument applied to
constant predictors.
\end{proof}

\paragraph{Where the content is.} The argument is three lines because
Theorem~\ref{thm:alpha} has already done the work: once two training targets induce the
\emph{same} edge law, no function of that law can separate them. The construction is the
claim; the proof is bookkeeping, and we do not present it as more than that.

Three remarks. (a) The bound is free of $n$: it is an information deficiency, not an
estimation problem, and $n=\infty$ does not help. (b) It uses only that the
\emph{distribution} of $\mathcal A$ is $\alpha$-free, so it also covers statistics given the
population law or the conditioning $c$. (c) The value $1/2$ is attained, so within this
constructed family the target marginal has zero decision value for estimating
$\rho^\ast$; this statement does not apply once the training pairing is observed.
(d) The endpoints are not straw men: $\alpha=0$ and $\alpha=1$ are the couplings in
routine use, and the bound forces every data-side predictor to be wrong by at least
$1/2$ on one of them; a practitioner holding an intermediate mixture cannot even tell
which regime they are in from the target marginal alone.

\paragraph{What the theorem does not say.} It does not say $\rho$ is unknowable.
$\rho$ uses the coupling, and the coupling is available: every training step of a flow
model draws a paired batch $(x_0^{(i)},x_1^{(i)})$. Estimating $\rho$ from those pairs
requires computation but no additional data or trained sampler, and can be done before
the one-step model exists. The theorem says the pairing carries information that the
target marginal omits.

%======================================================================
\section{Two design levers in the controlled study}
\label{sec:ladder}

\subsection{Lever 1: change the coupling}

Corollaries~\ref{cor:indep}--\ref{cor:det} give two endpoints, while the counterexample
after Cor.~\ref{cor:indep} shows that ``dependent'' alone is insufficient. Mini-batch OT
\citep{tong2024cfm}, reflow \citep{liu2023rectifiedflow}, and teacher-map distillation
often strengthen source--target mean dependence. ODE-free Neural Flow Matching already
proved mean collapse for independent endpoint regression and proposed OT pairings
\citep{shou2026nfm}; our distinction is that the exact condition is mean dependence,
not merely non-independence. Table~\ref{tab:map} is a controlled implementation study,
not an exhaustive classification of published methods.

\subsection{Lever 2: change the loss form}

The less explored channel is to keep the independent coupling and change the loss.
Let $q_c=f(\cdot,c)_\#\Ncal(0,I_d)$ denote the law the one-step model produces for
condition $c$, and let the training batch contain $M$ paired samples
$\{x_0^{(i)},x_1^{(i)},c^{(i)}\}_{i=1}^M$ with $G_i=f(x_0^{(i)},c^{(i)})$.

\begin{theorem}[Population limits of three objectives]
\label{thm:ladder}
Assume finite second moments, that each $p(\cdot\mid c)$ is a pushforward of $\Ncal(0,I_d)$
by a measurable map, and the standard empirical-measure convergence conditions.
Under the independent coupling, as $M\to\infty$:
\textbf{(L1)} the pointwise loss $\frac1M\sum_i\|G_i-x_1^{(i)}\|^2$ has unique
population minimiser $q_c=\delta_{m(c)}$, giving $\rho=0$;
\textbf{(L2)} the \emph{unbalanced} collection loss, with both $\arg\min$s taken
\emph{within the condition},
\[
  \frac1M\sum_i\min_j\|x_1^{(i)}-G_j\|^2+\frac1M\sum_j\min_i\|G_j-x_1^{(i)}\|^2
\]
converges to $\Eop_{y\sim p_c}[\min_{x\in\supp q_c}\|y-x\|^2]+\Eop_{x\sim q_c}[\min_{y\in\supp p_c}\|x-y\|^2]$,
which vanishes when the closed supports agree and is otherwise silent about weights
(supports identified only up to measure-zero/closure qualifications);
\textbf{(L3)} the \emph{balanced} collection loss, $\min_{\sigma}\frac1M\sum_i\|G_i-x_1^{(\sigma(i))}\|^2$
over bijections $\sigma$ within each condition, converges to $W_2^2(p_c,q_c)$, whose
unique minimiser is $q_c=p_c$ and $\rho=1$.
\end{theorem}
\begin{proof}[Proof sketch]
L1 is Cor.~\ref{cor:indep}. For L2, the two terms are non-negative and vanish exactly
when each support is contained in the other's closure; no term depends on the mass
assigned to a region once it is non-empty. For L3, minimising over bijections is
exactly the optimal-transport problem between the empirical measures, whose
$M\to\infty$ limit is $W_2^2$, minimised uniquely at $q_c=p_c$.
Appendix~\ref{app:proofs}.
\end{proof}

The L2/L3 distinction concerns population minimiser sets, not finite-network training
dynamics. Whether an optimiser uses the freedom in L2 is empirical. In our task it does
not: the observed flattening appears only when matching is pooled across conditions.

%======================================================================
\section{Experiments}
\label{sec:exp}

\paragraph{Common design.} Within each stated comparison we hold the synthetic data,
MLP width/depth, optimiser, batch size and per-stage update count fixed. Methods with a
teacher or reflow stage use more total compute, so this is not a compute-matched ranking.
Metrics are the spread ratio $\rho$ of \eqref{eq:rho} (with a $k$NN
estimator, Sec.~\ref{sec:estimator}), mode coverage (a mode counts as covered if any
sample falls within $0.6$ of its centre), and conditional sliced $W_2$ (cSW) normalised
by $\sqrt{\tr\Var(x_1\mid c)}$. Neural-training tables use five seeds; the analytic
$\alpha$ construction uses three independent Monte Carlo repetitions. All runs are CPU-only.

\subsection{\texorpdfstring{$\rho^\ast(\alpha)=\alpha^2$}{rho*(alpha) = alpha squared} with the data held fixed}
\label{sec:exalpha}

We instantiate \eqref{eq:alpha} with $\ImpC$ conditions, each an $\ImpK$-mode ring of
width $\sigma$, and $T$ the $512$-step Euler flow map of the exact conditional velocity
field. We sweep $\alpha\in\{0,\,0.25,\,0.5,\,0.75,\,1\}$ with a \emph{fresh} independent
sample at every level: had we instead reused one realisation, the data-side statistics
would be constant by construction and the test would restate the theorem instead of
testing it. The $k$NN estimate of $\rho$ moves
\ImpRhoZero{}$\to$\ImpRhoQuarter{}$\to$\ImpRhoHalf{}$\to$\ImpRhoThreeQ{}$\to$\ImpRhoOne{},
against the analytic $\alpha^2$ and its ideal local-neighbour finite-$k$ correction
$\alpha^2+(1-\alpha^2)/k$; the mean absolute deviation from the corrected prediction is
\ImpPredMad{} (criterion $<0.08$). Over the same sweep, the data-side statistics move by
at most \ImpDDev{} ($\cD$) and \ImpLamDev{} ($\cL$)---i.e.\ by Monte-Carlo noise---while
$\rho^\ast$ moves by $1.0$ (Fig.~\ref{fig:imposs}a). All five pre-registered criteria
pass (verdict \ImpVerdict).

\begin{figure*}[t]
\centering
\includegraphics[width=0.74\textwidth]{../figures/fig_impossibility.pdf}
\caption{\textbf{Target marginal versus training pairing.} (a) $\rho$ against $\alpha$ on the
family of Thm.~\ref{thm:alpha}: measured (circles) versus the analytic $\alpha^2$ (solid)
and its finite-$k$ correction (dashed); on the right axis two data-side statistics, flat
while $\rho$ traverses $[0,1]$. (b) Controlled method implementations with cross-seed SD:
a finite comparison, not a universal dichotomy.}
\label{fig:imposs}
\end{figure*}

\subsection{A battery of data-side statistics is flat in \texorpdfstring{$\alpha$}{alpha}}
\label{sec:exminimax}

Theorem~\ref{thm:minimax} is exact and needs no experiment; what needs checking is its
empirical premise---that statistics one would actually reach for are, in fact, flat. We
test seven: $\cD$ (unexplained conditional-variance fraction), $\cL$ (Gaussian deficit),
the number of clusters selected by Calinski--Harabasz, mean nearest-neighbour distance,
the $k$-means between/within variance ratio at $K=8$, excess kurtosis, and the marginal
trace variance.

Because these statistics differ by two orders of magnitude in their own sampling noise,
a fixed relative threshold tests the noise rather than the $\alpha$ effect: run with
na\"ive $k$-means initialisation, the $K=8$ ratio varies from $15$ to $36$ \emph{across
seeds at the same $\alpha$}. We therefore test with a one-way ANOVA over
$\alpha$ with seeds as replicates (and with restarted $k$-means++). At level
$\MinimaxPAlpha$ with five seeds per level, no statistic shows a detectable $\alpha$
effect; the largest relative deviation from $\alpha=0$ is \MinimaxDevMax{}. By contrast the paired estimator's mean
absolute deviation from $\alpha^2$ is \MinimaxPairedMad{}---\MinimaxRiskRatio$\times$
smaller than the $\MinimaxConst$ that Theorem~\ref{thm:minimax} imposes on \emph{any}
data-only predictor.

Sample size does not rescue the data side. Increasing $n$ from \LadderNFirst{} to
\LadderNLast{} leaves the $\alpha=0$-versus-$\alpha=1$ difference of every
$[0,1]$-scaled statistic below \LadderWorstAbsDiff{}---that is, at most $0.3\%$ of the
$1.0$ that a useful predictor must resolve---while the paired estimator, with the same
budget of samples, tracks $\alpha^2$. If a data-side statistic carried a real
$\alpha$-signal, its difference would converge to a nonzero constant as $n$ grows; it
does not. (Two of the seven, the Calinski--Harabasz count and the $k$-means variance ratio, are
ratio-scaled, so absolute thresholds do not apply; their $\alpha$-independence rests on
the scale-free ANOVA above.)

\subsection{What does decide: the method map}
\label{sec:exmap}

Table~\ref{tab:map} runs six method families plus two one-step-only variants on the
same conditional ring mixture. In these implementations, both collapsing rows use
independent targets with pointwise $L^2$: flow matching at $N=1$ gives
$\rho=\MMIndepOneRho\pm\MMIndepOneStd$ with coverage $\MMIndepOneCov$/$8$, and a direct
one-step regression gives $\MMLtwoOneRho$. Removing \emph{either} conjunct removes the
collapse in this task---by the coupling (mini-batch OT \MMOtOneRho, reflow \MMReflowOneRho,
distillation \MMDistillOneRho) or by the loss form with the coupling left independent
(pooled bidirectional Chamfer \MMChamferRho; re-measured as L2a in
Table~\ref{tab:ladder} at \LLChamferPooledRho, where its support/weight failure is
dissected). The same field that collapses at $N=1$ recovers at
$N=32$ ($\MMIndepManyRho$), showing that the same learned velocity field can recover the
distribution when integrated. This finite method set does not establish the biconditional
that was claimed in an earlier draft.

\begin{table}[t]
\centering
\caption{Controlled method implementations. Data, network width/depth, optimiser, batch
size and per-stage step count are fixed; total compute is not equal because reflow and
distillation include a teacher stage. The rows support the stated implementations only;
they do not prove a converse over all one-step methods.}
\label{tab:map}
\vskip 0.05in
\tblstyle{
\begin{tabular}{llccc}
\toprule
method (all at NFE$=1$) & coupling & loss & $\rho$ & modes / $8$ \\
\midrule
flow matching & independent & pointwise & \MMIndepOneRho{}$\pm$\MMIndepOneStd & \MMIndepOneCov \\
\quad same field, NFE$=32$ & independent & pointwise & \MMIndepManyRho{}$\pm$\MMIndepManyStd & \MMIndepManyCov \\
\quad + mini-batch OT & $\approx$det. & pointwise & \MMOtOneRho{}$\pm$\MMOtOneStd & \MMOtOneCov \\
\quad + reflow & deterministic & pointwise & \MMReflowOneRho{}$\pm$\MMReflowOneStd & \MMReflowOneCov \\
distillation of the $32$-step map & deterministic & pointwise & \MMDistillOneRho{}$\pm$\MMDistillOneStd & \MMDistillOneCov \\
one-step regression & independent & pointwise & \MMLtwoOneRho{}$\pm$\MMLtwoOneStd & \MMLtwoOneCov \\
one-step, min-of-$M$ & independent & collection & \MMMinMOneRho{}$\pm$\MMMinMOneStd & \MMMinMOneCov \\
one-step, bidirectional Chamfer (pooled) & independent & collection & \MMChamferRho{}$\pm$\MMChamferStd & \MMChamferCov \\
\bottomrule
\end{tabular}}
\end{table}

\subsection{Loss comparison: matching scope matters in this task}
\label{sec:exladder}

We train four one-step models on the \emph{same} independent coupling, changing only the
loss: pointwise $L^2$ (L1); a bidirectional Chamfer with both $\arg\min$s over the
\emph{whole batch}, as such losses are usually written (L2a); the same objective confined
to each condition (L2b); and a balanced within-condition assignment (L3). L2b and L3 differ
in exactly one respect---whether the assignment is a bijection---the single variable
Thm.~\ref{thm:ladder} isolates; L2a versus L2b isolates a second, the \emph{scope}. The
conditions are four rings of radius $1.5,1.9,2.3,2.7$, so $\tr\Var(x_1\mid c)$ ranges by a
factor of $3.1$, making ``support correct, weights wrong'' directly visible.

\begin{table}[t]
\centering
\caption{Loss comparison at one step, independent coupling held fixed. L1 collapses. L2a
(Chamfer pooled over the batch) escapes and has similar aggregate $\rho$ to L2b, yet
flattens conditional spread in this task; L2b, matched within each condition, restores
the evaluated spread; L3, balanced within each condition, has the lowest conditional sliced $W_2$.
$\mathrm{disp}=\mathrm{std}_j(\rho_j)$; $|\rho_j-1|$ is the mean absolute per-condition
spread error. L2b and L3 differ only in whether the assignment is a bijection.}
\label{tab:ladder}
\vskip 0.05in
\tblstyle{
\begin{tabular}{lccccc}
\toprule
loss & $\rho$ & modes / $8$ & disp & $|\rho_j\!-\!1|$ & cSW \\
\midrule
L1 pointwise $L^2$ & $\LLLtwoRho{}\pm\LLLtwoStd$ & \LLLtwoCov & \LLLtwoDisp & \LLLtwoAbsErr & \LLLtwoCsw \\
L2a unbalanced, pooled over cond. & $\LLChamferPooledRho{}\pm\LLChamferPooledStd$ & \LLChamferPooledCov & \LLChamferPooledDisp & \LLChamferPooledAbsErr & \LLChamferPooledCsw \\
L2b unbalanced, within cond. & $\LLChamferRho{}\pm\LLChamferStd$ & \LLChamferCov & \LLChamferDisp & \LLChamferAbsErr & \LLChamferCsw \\
L3 balanced, within cond. & $\LLBalRho{}\pm\LLBalStd$ & \LLBalCov & \LLBalDisp & \LLBalAbsErr & \LLBalCsw \\
\bottomrule
\end{tabular}}
\end{table}

Table~\ref{tab:ladder} does \emph{not} confirm the ladder as stated.
Fig.~\ref{fig:ladder}a plots generated against true conditional spread. L1 sits at
zero (collapse). L2a escapes, and on the aggregate it is indistinguishable from the
method that gets the spread right: $\rho=\LLChamferPooledRho$ for L2a against
\LLChamferRho{} for L2b. The aggregate is simply blind here---what exposes L2a is that its
spread is nearly \emph{horizontal} (coefficient of variation \LLChamferPooledGenCV{}
against a true \LLTrueCV), i.e.\ the dispersion of $\rho_j$ across conditions:
\LLChamferPooledDisp{} for L2a against \LLChamferDisp{} for L2b. That horizontal reading
does not survive the control: L2b, differing from L2a only in matching within each
condition, returns to the diagonal, so the flattening came from the \emph{pooling},
not from the missing bijection. L2b and
L3 both track the diagonal, separated only by what the metric rewards: L3 has the lowest
conditional sliced $W_2$ (\LLBalCsw{} against \LLChamferCsw, a factor of \LLRatioCsw) while
L2b is nominally closer on the scale ratio ($|\rho_j-1|$ \LLChamferAbsErr{} against
\LLBalAbsErr). A single aggregate number cannot substitute for the conditional statistic: it reports
\LLChamferPooledRho{} for the pooled variant (scale error \LLChamferPooledAbsErr) against
\LLChamferRho{} for the within-condition variant (scale error \LLChamferAbsErr).

\paragraph{A prediction that failed.} We expected that dropping balancedness at fixed
scope (L2b vs.\ L3) would flatten the conditional spread; it did not, and the ladder's
pre-registered verdict is \LLVerdict{}---the check ``balancedness reduces weight error''
fails outright. In this experiment, the variable associated with flattening is the scope
of the $\arg\min$. Thm.~\ref{thm:ladder} itself concerns population objectives and does
not predict finite-training dynamics or pooled matching.

\begin{figure*}[t]
\centering
\includegraphics[width=0.70\textwidth]{../figures/fig_loss_ladder.pdf}
\caption{\textbf{Controlled loss comparison.} Independent coupling held fixed; only the
loss form changes. Error bars are cross-seed SD. (a) Generated versus true per-condition
total variance over four conditions; the diagonal certifies this variance statistic only.
L1 (pointwise $L^2$) sits at zero; L2a (pooled Chamfer, whole-batch $\arg\min$) is nearly
\emph{horizontal}---all eight evaluated modes are covered but spread is flattened; L2b
(matched \emph{within} each condition) returns to the diagonal; L3 (balanced, within
condition) is closest in sliced $W_2$. Flattening tracks the scope of the matching, not
the missing bijection. (b) Conditional sliced $W_2$ and mean spread error.}
\label{fig:ladder}
\end{figure*}

\paragraph{A corrected mechanism: min-of-$M$.} min-of-$M$ (IMLE) also escapes
($\rho=\MMMinMOneRho$, coverage $\MMMinMOneCov/8$, against $\MMLtwoOneRho$ for pointwise
$L^2$) once its $M$ candidates come from \emph{independent} source noise. An earlier claim
that it did not escape was an implementation artefact---repeating one source draw makes the
objective literally pointwise $L^2$ (Appendix~\ref{app:corrections}(iii)).

\subsection{Estimating \texorpdfstring{$\rho$}{rho} from paired samples}
\label{sec:estimator}

$\rho^\ast$ is the variance of a conditional mean, so the natural estimator is $k$NN
regression of $x_1$ on $x_0$ within each condition. Its bias under the ideal
zero-neighbourhood approximation, which we use as a check: under an independent coupling, the
$k$-neighbour average of i.i.d.\ targets has variance $\Var(x_1\mid c)/k$, so
$\hat\rho_{\mathrm{kNN}}(k)\approx\rho^\ast+(1-\rho^\ast)/k$, which at $\rho^\ast=0$
is the $1/k$ law. Measured $k\cdot\hat\rho$ under the independent coupling is
\CondKTimesOne{},\,\CondKTimesTwo{},\,\CondKTimesFive{},\,\CondKTimesTen{},\,\CondKTimesTwenty{}
at $k=1,2,5,10,20$: the law is accurate for $k\le10$ and degrades to \CondKTimesTwenty{}
by $k=20$, where the zero-neighbourhood approximation weakens.

\subsection{Real-data confirmation: endpoint regression on MNIST}
\label{sec:mnist}

Everything so far is synthetic. We repeat the endpoint experiment on MNIST
$16{\times}16$ images \citep{lecun1998mnist}, with $x_0\sim\Ncal(0,I_{256})$ and four
couplings that share the source and differ only in how a target $x_1$ is paired with
it---(i)~\emph{independent} ($x_1=x\tilde{}$, an image); (ii)~\emph{dependent but
mean-independent}, $x_1=\mu+(1+0.8\tanh x_{0,1})\,(x\tilde{}-\mu)$ with $\mu$ the data
mean, so $\Eop[x_1\mid x_0]=\mu$ exactly while the conditional variance varies with $x_0$
by up to $(1.8)^2$; (iii)~\emph{mini-batch OT}; and (iv)~\emph{sorted quantile pairing}:
within each batch, sources are sorted by $x_{0,1}$, images by their projection on the
data's first principal axis, and ranks matched---$O(m\log m)$, no assignment solver---so
$\Eop[x_1\mid x_0]$ is a deterministic function of $x_{0,1}$, at \MnistDSeconds{} s per
seed against \MnistCSeconds{} s for per-batch OT. We train the same small MLP with $L^2$
per coupling, \MnistNSeeds{} seeds; (i)/(ii) stop at the $\mu$-baseline (the population
optimum) so memorising finite pairs cannot fake the result.
The distance correlation \citep{szekely2007distancecorr} of (ii) is \MnistBDcor{} against \MnistADcor{} for (i):
the coupling is measurably dependent, \emph{and it collapses anyway}. The trained
map's realised spread is $\rho=\MnistARhoTr{}$ for (i) and $\MnistBRhoTr{}$ for
(ii)---both an order of magnitude below (iii)'s $\MnistCRhoTr{}$---and the mean
output of (i)/(ii) is the grey mean image (relative error \MnistAMuErr{} and
\MnistBMuErr{}) with the worst sliced-$W_2$ fidelity (\MnistACsw{}/\MnistBCsw{}
versus (iii)'s \MnistCCsw{}).
Coupling (iv) is the constructive counterpart: the cheapest pairing that forces
the conditional mean to depend on $x_0$. The constructed dependence is detected
(distance correlation \MnistDDcor{} against \MnistADcor{} for (i)), the trained map
does not collapse ($\rho=\MnistDRhoTr{}$, an order of magnitude above the (i)/(ii)
floor), and fidelity improves (sliced-$W_2$ \MnistDCsw{} versus
\MnistACsw{}/\MnistBCsw{}) yet stays well short of full OT (\MnistCCsw{}): matching
quantiles of a \emph{single} statistic recovers only the variance fraction the first
principal axis explains. The four couplings thus form a ladder of mean
dependence---(i)/(ii) no mean dependence, (iv) one statistic, (iii) complete---and the
trained
map's spread climbs it in lockstep: on these four MNIST couplings the criterion
separates collapse from recovery and points to the dependent pairing as the remedy.
Across-seed dispersion is small: the largest standard deviation among the diagnostic
means reported above is \MnistMaxStd{} (\MnistMaxStdOf{}), over \MnistNSeeds{} seeds.
One limitation: the data-side $k$NN estimator is uninformative at this dimension
($d{=}256$, $n{\approx}10^3$; $\approx 0$ for \emph{all four} couplings, a
curse-of-dimensionality failure), so the criterion is validated through trained maps,
with the estimator's operating regime as in Sec.~\ref{sec:estimator}.

%======================================================================
\section{A negative result about our own earlier proposal}
\label{sec:negative}

An earlier version of this line of work advanced $\cL$, a null-calibrated conditional
mode-separation score, as a \emph{modality detector}. We report the experiment that
kills that claim. On a fixed battery of ten population distributions the Gaussian
deficit (the quantity behind $\cL$) ranges from $\LamGauss$ (Gaussian) to
$\LamTwoModes$ (two modes), with $\LamBall$ (uniform ball), $\LamLaplace$ (Laplace),
$\LamHetero$ (heteroscedastic), $\LamRingEight$ (eight-mode ring), $\LamStudentT$
(Student-$t_3$), $\LamLogNorm$ (log-normal), $\LamExpSkew$ (exponential) and
$\LamBanana$ (banana) in between. It has the correct analytic ceiling of
$1-2\sqrt{2/\pi}+1$, matched by the finite-sample estimator to $\LamCeilingEstErr$ at
$n=\LamCeilingNMin$, and is stable across sample sizes (largest disagreement between
$n=2000$ and $n=2\times10^{5}$: \LamConsistencyMaxDiff). But it \emph{sorts a
single-mode banana above an eight-mode ring}: $\LamBanana>\LamRingEight$. A statistic
that ranks one mode above eight cannot be called a modality detector, whatever its
construct validity on the remaining nine distributions (rank correlation
$\LamRankCorrOT$ against a brute-force optimal-transport computation,
$\LamRankCorrConstruct$ against a constructed error measure).

We keep $\cL$ as a descriptive index of departure from Gaussianity and drop the modality
claim. This is not incidental: it is the same lesson as Sec.~\ref{sec:imposs} in
miniature---any scalar summary of a conditional law, ours included, is a projection, and
the projection that separates the cases you had in mind need not separate the cases that
occur.

%======================================================================
\section{Related work}
\label{sec:related}

\paragraph{Correlation-ratio dependence.} The scalar $\rho^\ast$ is the multivariate
correlation ratio---the fraction of target variance explained by the conditional mean
\citep{renyi1959,sampson1984}. We use this classical quantity to state the exact population
criterion for endpoint mean collapse and claim no new measure; dependence alone is
insufficient, since a dependent coupling can be conditionally mean-independent.

\paragraph{Few-step generative models.} Consistency models \citep{song2023consistency},
consistency flow matching \citep{yang2024consistencyfm}, shortcut models
\citep{frans2024shortcut}, mean flows \citep{geng2025meanflows}, progressive
\citep{salimans2022progressivedistill} and distribution-matching \citep{yin2024dmd}
distillation, and InstaFlow \citep{liu2024instaflow} target few-step sampling. Our
contribution is not a sampler but a statement about which design choice governs the
outcome, plus a proof that data cannot substitute for it.

\paragraph{Coupling and straightness.} Rectified flow \citep{liu2023rectifiedflow} shows
straightening the coupling makes one step viable; stable diffusion~3 \citep{esser2024sd3}
shows the training-time distribution matters as much as the sampler; non-Gaussian sources
reduce path crossing in physical simulation \citep{onestepfm2026}. This literature
\citep{tong2024cfm,liu2023rectifiedflow,pooladian2023multisample} already knows the
coupling matters---Thm.~\ref{thm:optimal} is the $L^2$ projection and total-variance
identity---and ODE-free Neural Flow Matching proves mean collapse for independent endpoint
regression and proposes optimal-transport pairing \citep{shou2026nfm}. Our narrower
correction: non-independence is not an if-and-only-if condition, conditional mean
dependence is, and Thm.~\ref{thm:minimax} formalises why the target marginal alone cannot
reveal it.

\paragraph{One-step policies in RL.} \citep{nguyen2026bfq,li2026ofp,mvp2026,lps2026,
flame2026,fan2026} report strong one-step policies, but the advantage disappears when the
observation is weakened or the action chunk lengthened
\citep{li2026ofp,letitbesimple2026}; \citep{letitbesimple2026} calls this a
condition--target \emph{structure} property formalised as an irreducible velocity loss. Our
synthetic analysis isolates a complementary variable---the source--target pairing---without
claiming downstream control improvements.

\paragraph{Collection-level losses and diagnostics.} Implicit maximum likelihood estimation
\citep{li2018imle} and Chamfer-style objectives are standard tools for avoiding
mode-averaging, typically presented as interchangeable; Thm.~\ref{thm:ladder} separates
them by the \emph{scope} of the matching, and Sec.~\ref{sec:exladder} shows the scope---not
the balance---flattens the conditional spread. \citep{li2026nofreelunch} gives a
pre-training, conditional-spread-based few-step diagnostic ($q^\ast/\sigma^\ast$) for
conformal prediction; $\sqrt{\cD}$ is our version of that axis with a different
normalisation, and we prove the diagnostic cannot be extended to the general one-step
retention question.

%======================================================================
\section{Conclusion and limitations}
\label{sec:conclusion}

For pointwise $L^2$ endpoint regression, one-step mean collapse is governed exactly by
conditional mean dependence, not by dependence in general. In an explicit coupling family
the target marginal is identical while $\rho^\ast$ spans $[0,1]$, giving minimax
absolute-error risk $1/2$ to any target-marginal-only estimator; paired training samples
restore the missing information. Our collection-loss experiments show that the scope of matching matters on the controlled ring task, without
establishing a universal ranking.

\paragraph{Limitations.} (1) The impossibility is proved for an explicit family---not
contrived ($\alpha=0,1$ are the couplings in use) but not literally a practitioner's
target---and bounds the worst case. (2) Measurements are CPU-scale \emph{synthetic}
families; we have not validated on D4RL \citep{fu2020d4rl}, OGBench
\citep{park2025ogbench}, LIBERO \citep{liu2023libero}, or a VLA. (3) Balanced assignment
costs a Hungarian solve per batch per condition, with unstudied scaling. (4) $\rho$
measures conditional spread, not task reward; Appendix~\ref{app:downstream} pre-registers
the tests that would refute the criterion.


\section*{AI use statement}
Generative AI systems were used to propose and refine the hypothesis, locate and summarise
literature, critique mathematical claims, implement and audit code, suggest experiments,
and draft text and figures. The authors are responsible for the complete submission.
Mathematical claims and aggregate values were independently checked during the present
audit; the remaining result-provenance limitation is disclosed below and in the audit log.

\section*{Reproducibility statement}
Appendices~\ref{app:runtime} and~\ref{app:estimators} give the data, compute and execution
protocol. The supplement contains the source code, per-run result files, table/figure
generators, and regression tests for the failure modes in
Appendix~\ref{app:corrections}. No reported result is hand-copied: every number in a
table, a figure or a quantitative claim is a macro generated from the released JSON, and
the per-seed files behind it are released. Numerals that appear only in this prose are
analytic constants, a priori design choices, or order-of-magnitude diagnostics---the
$512$-step solver, the ring radii, the across-seed spread of the $k$-means ratio---never a
headline quantity. Every per-seed result file carries a schema version and a
SHA-256 fingerprint of the frozen protocol files; merging new seeds requires an
exact fingerprint match, so all reported seeds come from one code revision.

\section*{Ethics statement}
This is an analysis of when one-step generative policies suffice. It uses only synthetic data
with exactly controlled ground truth, involves no human subjects, and releases no new
dataset. Where our own earlier proposal failed we say so (Sec.~\ref{sec:negative}).

\bibliography{references}
\bibliographystyle{iclr2027_conference}

\appendix
\section{Proofs}
\label{app:proofs}

\subsection{Theorem~\ref{thm:optimal}}
Since $\Eop\|x_1\|^2<\infty$, the minimiser of $\Eop\|f(x_0,c)-x_1\|^2$ over measurable
$f$ with finite second moment is the conditional expectation, by the $L^2$ projection
theorem applied conditionally on $(x_0,c)$. For
the range, condition on $c$ and apply the law of total variance to the scalar
components:
\[
  \Var(x_1\mid c)=\Eop\big[\Var(x_1\mid x_0,c)\bigm|c\big]
  +\Var\big(\Eop[x_1\mid x_0,c]\bigm|c\big)\;\succeq\;
  \Var\big(\Eop[x_1\mid x_0,c]\bigm|c\big)
\]
in the positive-semidefinite order, so the trace of the conditional-mean term is at most
the trace of $\Var(x_1\mid c)$ and $\rho\le1$; $\rho\ge0$ is immediate. Equality holds
iff $\Var(x_1\mid x_0,c)=0$ a.s., i.e.\ iff $x_1$ is a measurable function of
$(x_0,c)$. At the other endpoint, the expected trace of
$\Var(\Eop[x_1\mid x_0,c]\mid c)$ is zero iff the conditional mean is constant in
$x_0$ almost surely; taking its conditional expectation identifies that constant as
$\Eop[x_1\mid c]$. This proves both equality statements in
Thm.~\ref{thm:optimal}.

\subsection{Theorem~\ref{thm:minimax}, full statement}
The proof in the main text is complete; we record two extensions. \emph{(i)
Randomisation does not help}: if $\mathcal A$ has access to auxiliary randomness
$U\indep\text{data}$, the law of $(\text{data},U)$ is still $\alpha$-free, so the same
Jensen argument applies to $\bar a=\Eop[\mathcal A]$. \emph{(ii) Knowledge of
$p(c,x_1)$ does not help}: a statistic of the population law is a constant, and
constants are covered by the second equality in the theorem. \emph{(iii)} The same
argument gives $\inf_{\mathcal A}\sup_\alpha\Eop(\mathcal A-\rho^\ast)^2=1/4$ for
squared loss, attained at $1/2$ as well.

\subsection{Theorem~\ref{thm:ladder}, L3 limit}
For a batch of $M$ samples within one condition, minimising
$\frac1M\sum_i\|G_i-x_1^{(\sigma(i))}\|^2$ over bijections $\sigma$ is exactly the
optimal-transport problem between the empirical measures $\hat q_c$ and
$\hat p_c$ with quadratic cost; its value is $W_2^2(\hat q_c,\hat p_c)$. As
$M\to\infty$ the empirical measures converge weakly and
$W_2^2(\hat q_c,\hat p_c)\to W_2^2(q_c,p_c)$ under the usual moment conditions. The
unique minimiser over probability measures is $q_c=p_c$; the minimiser over
\emph{pushforwards of $\Ncal(0,I_d)$ by a measurable map} is any $f$ with
$f(\cdot,c)_\#\Ncal(0,I_d)=p(\cdot\mid c)$, and every such $f$ has $\rho=1$.

\subsection{Theorem~\ref{thm:ladder}, L2 limit}
Write $d_{q_c}(y)=\min_{x\in\supp q_c}\|y-x\|$ and $d_{p_c}(x)=\min_{y\in\supp p_c}\|x-y\|$,
and assume the supports are compact so that the empirical measures converge weakly, as in
the L3 case. For fixed $y$ we have $\min_j\|y-G_j\|\to d_{q_c}(y)$ almost surely: the lower
bound $\min_j\|y-G_j\|\ge d_{q_c}(y)$ holds for every $M$, and for every $\varepsilon>0$ an
$\varepsilon$-net of $\supp q_c$ is almost surely eventually occupied, whence
$\min_j\|y-G_j\|\le d_{q_c}(y)+\varepsilon$ for all large $M$. Both $y$ and $G_j$ range
over compact sets, so the squared integrand is bounded; dominated convergence then gives
the limit $\Eop_{y\sim p_c}[d_{q_c}(y)^2]$ of the first term, and the second is symmetric.
The population functional is therefore a sum of two non-negative terms, vanishing exactly
when each closed support is contained in the other's. It carries no term penalising the
mass a realisation assigns within a non-empty region, so it is minimised by every $q_c$ with
$\supp q_c=\supp p_c$ and is otherwise silent about weights---the L2/L3 separation.

\subsection{Multi-step ratio values of Sec.~\ref{sec:setup}}
For the Gaussian-mixture construction of Sec.~\ref{sec:setup} the endpoint ratio
$\rho_N$ at step counts $N=1,2,4,\dots,256$ is
\[
\begin{array}{lccccccccc}
N & 1 & 2 & 4 & 8 & 16 & 32 & 64 & 128 & 256\\[2pt]
\rho_N & \MSRhoOne & \MSRhoTwo & \MSRhoFour & \MSRhoEight & \MSRhoSixteen &
\MSRhoThirtyTwo & \MSRhoSixtyFour & \MSRhoOneTwentyEight & \MSRhoTwoFiftySix
\end{array}
\]

\section{Corrections we made during the study}
\label{app:corrections}
We record the errors we found and fixed, because each is a trap a reader
re-implementing this will also hit.

\paragraph{(i) Never use a guessed effect size as a pre-registered criterion.}
Our first version of the $\cL$ validation required $\Lambda(\Delta=6)\ge0.50$; it
failed, but the failure said nothing about the statistic---the analytic ceiling of the
quantity is $0.404$, so the criterion was unsatisfiable. Likewise the first version of
the loss-ladder criterion required $\mathrm{disp}(\text{Chamfer})>2\times
\mathrm{disp}(\text{balanced})$ and failed at a $40$-step smoke test before the models
had converged. Effect sizes depend on how far optimisation has gone and are not
knowable in advance; we now pre-register only \emph{directional} and \emph{qualitative}
predictions and report magnitudes descriptively.

\paragraph{(ii) Test invariance against the statistic's own noise.}
Our first invariance check flagged the $k$-means between/within ratio as
$\alpha$-dependent (relative deviation $0.16$). It is not: with na\"ive random
initialisation the statistic varies from $15$ to $36$ \emph{across seeds at fixed
$\alpha$}. A fixed relative threshold tests noise, not signal. We switched to a
one-way ANOVA with seeds as replicates and to restarted $k$-means++.

\paragraph{(iii) Two collection-level objectives could not test their own claim.}
\emph{(a) min-of-$M$.} Our first implementation of the min-of-$M$ row built its $M$
candidates by repeating a single source draw, $x_0^{(i)},\dots,x_0^{(i)}$. A one-step
network is a deterministic map, so the $M$ outputs coincide \emph{at every training
state}---not only at collapse; the $\arg\min$ is therefore constant and the loss is the
pointwise $L^2$ loss on the same batch. The two code paths were identical to the last
bit: $\max|\Delta w|=0$ after $200$ steps of each, and in the full runs the spread ratio
agreed to all $17$ printed digits ($0.00010386879583898434$ under both). The original row
therefore measured nothing about IMLE. The corrected objective draws $M$
\emph{independent} $x_0$ per target, which is the original IMLE semantics, and
Table~\ref{tab:map} uses the regenerated row. \emph{(b) the unbalanced Chamfer row.} Our
first implementation took both $\arg\min$s over the \emph{whole batch}, whereas the
balanced row took its assignment \emph{within each condition}. The comparison therefore
varied two things at once---balancedness and conditioning scope---while
Thm.~\ref{thm:ladder}(L2--L3) isolates the first. The corrected row matches within each
condition (pooling is a \emph{weaker} constraint, since a minimum over a superset is
smaller, so this change strengthens the support requirement rather than manufacturing
the support recovery we observe). The general lesson for both: an objective whose entire
purpose is to \emph{choose} among candidates should assert in code that the candidate
sets and the choice are the ones the theorem talks about.

\paragraph{(iv) $\cL$ is not a modality detector.}
Sec.~\ref{sec:negative}. It sorts a single-mode banana ($\LamBanana$) above an
eight-mode ring ($\LamRingEight$).

\paragraph{(v) A tolerance half the mode spacing silently inverts the mode metric.}
Defining ``on a mode'' as within half the nearest-centre distance shrinks the
tolerance below the mode width as $K$ grows; at $K=1$ it is undefined and hides the
collapse. We use a $\sigma$-scale ball measured from mode \emph{width}.

\paragraph{(vi) Marginal sliced $W_2$ conflates the one-step floor with mode
collapse.} The ratio SW@$1$/SW@$32$ reports a penalty even for an under-fit field that
has none to report. The conditional ratio cSW@$1$/cSW@$32$ is the one the floor
bounds, and we use it throughout.

\paragraph{(vii) The no-free-parameter prediction used the $W_1$ constant inside a
$W_2$ argument.} An earlier draft used $\sigma\sqrt{2/\pi}$ (the $W_1$ distance,
equivalently the mean absolute deviation) where the $W_2$ distance is the scale gap
$\sigma$, giving $\kappa_d=\sqrt{2/(\pi d)}$ instead of $1/\sqrt d$.

\paragraph{(viii) ``Coverage exactly zero'' is not a satisfiable criterion.}
The method map pre-registered ``the independent coupling covers $0$ of $8$ modes at
$N=1$''. The theory (Cor.~\ref{cor:indep}) is about the \emph{exact} optimum, which does
sit on $m(c)$ and covers exactly $0$; a \emph{trained} network cannot attain the exact
optimum, and over $2000$ evaluation samples an occasional point falls inside a mode's
tolerance ball, giving a mean coverage of $0.08/8$. The criterion as written asked for
numerical perfection. We relaxed it to at most $1$ of $8$ modes---still two orders of
magnitude away from the passing side ($8/8$)---and record the change rather than
quietly keeping the original verdict.

\section{Runtime and reproducibility}
\label{app:runtime}
Every number in the main text is produced by the scripts under \texttt{code/} from the
raw per-run JSON files under \texttt{results/} and \texttt{logs/}; the paper never
contains a hand-copied statistic. Numbers in the theory sections come from
\texttt{paper/numbers\_theory.tex}, generated by
\texttt{code/analysis/make\_theory\_macros.py}, whose every macro carries a comment
naming the JSON field it came from.

\section{Estimators, and why they are deliberately classical}
\label{app:estimators}
Every diagnostic in this paper is built from standard, assumption-light
non-parametric tools, so that a reader can re-implement it without training any
auxiliary model. The conditional mean behind $\rho$ is estimated by $k$NN regression
\citep{cover1967nn}; the mode count and the between/within variance ratio use $k$-means
\citep{lloyd1982,macqueen1967} selected by the Calinski--Harabasz criterion
\citep{calinski1974}; the conditional spread is measured by sliced $W_2$
\citep{kolouri2019gsw}. We use these classical estimators on purpose: they carry almost
no assumptions, need no hyper-parameter search, and are \emph{decoupled from the
generative model being diagnosed}, which is what lets $\rho$ be defined and computed
before any one-step model exists (Sec.~\ref{sec:estimator}). The one consequence to
keep in mind is the $k$NN bias in \eqref{eq:rho}: all statements are about the
population quantity $\rho^\ast$, and we report the estimator's analytic bias rather
than correcting it away.

\section{Minimal downstream validation design}
\label{app:downstream}
The criterion makes a falsifiable prediction outside the laboratory tasks of this
paper, and testing it does not require the full apparatus of policy benchmarking. We
pre-register the following minimal protocol so that a follow-up (ours or independent)
can be judged against a fixed standard.

\textbf{Task.} One offline continuous-control suite with weak and strong conditioning
variants (e.g.\ a single LIBERO task suite under two instruction-conditioning strengths,
or the weakest-conditioning split of an offline dataset). No robot hardware is needed;
the pairing is a training-time choice.

\textbf{Arms.} Same backbone, same loss, same schedule; the only difference is the
coupling: (i)~independent, (ii)~paired with the sort-based or OT scheme of
Sec.~\ref{sec:mnist}. Compute $\hat\rho$ from the paired batches before training
(Sec.~\ref{sec:estimator}), at the training dimension.

\textbf{Pre-registered predictions.} (P1) the arm with lower $\hat\rho$ collapses to
its conditional mean and fails on the weak-conditioning split while the arm with
higher $\hat\rho$ does not; (P2) the failure gap grows with the conditioning gap,
qualitatively tracking the $\rho^\ast$ gap; (P3) the mean-dependence ordering of
Sec.~\ref{sec:mnist} reproduces on the policy's action distribution
($\rho$ of the trained policy's one-step map).

\textbf{Falsification.} If both arms collapse despite a large $\hat\rho$ gap, or the
high-$\hat\rho$ arm fails where the low-$\hat\rho$ arm succeeds, the criterion's
transfer to policy pipelines is refuted and the paper's practical claim should be
retreated to the controlled setting. Ten or more seeds per arm with bootstrap
intervals are the minimum for the comparison to be interpretable.

Two further failure modes would challenge the central claims directly. First, if the
paired estimator of Sec.~\ref{sec:estimator} fails to track an independently computed
population $\rho^\ast$ wherever both are available, the measured $\rho$ curves lose their
interpretation as conditional-spread retention. Second, if the mean-independence
criterion fails under the finite-moment and population-optimum assumptions of
Thm.~\ref{thm:optimal}---for instance if a pointwise $L^2$ regressor retains conditional
spread when the conditional mean does not depend on $x_0$---then the exact criterion is
not exact. Data-side estimators that survive the high-dimensional regime where the current
$k$NN estimator fails (Sec.~\ref{sec:mnist}) are the prerequisite for the first test.

\section{Scope and assumptions}
\label{app:scope}
\begin{itemize}
  \item \textbf{Field:} machine learning --- generative modelling for decision making,
  as an \emph{analysis and diagnosis} contribution, not a new sampler.
  \item \textbf{Tasks:} synthetic conditional ring mixtures with closed-form ground
  truth; a real-image endpoint-regression study on MNIST $16\!\times\!16$
  \citep{lecun1998mnist} in which the coupling is constructed by us; a multi-goal
  continuous-control task used as an implementation smoke test only. Downstream
  policy-benchmark validation is specified but not executed
  (Appendix~\ref{app:downstream}).
  \item \textbf{Datasets:} synthetic families plus the public MNIST training images;
  no proprietary data, no robot hardware, no pretrained backbone.
  \item \textbf{Compute:} CPU only (a $20$-core workstation, PyTorch \texttt{cpu}
  build). No GPU was used at any point.
  \item \textbf{Attribution:} we do not reuse any author repository; each component is
  re-implemented from the published objective. Any error is ours.
  \item \textbf{Target venue:} ICLR 2027, double-blind. Submission metadata is not a
  scientific claim and is therefore not cited.
\end{itemize}

\end{document}
